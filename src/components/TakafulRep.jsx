...
function TakafulRep() {
    ...
    const fetchTakafulReports = async () => {
        ...
        try {
            ...
            const res = await axios.get(`http://localhost:3002/api/v1/TakafulAccidentReport/all`, {
                headers: { token }
            });
    ...
    const handleDelete = async (id) => {
        ...
        const token = `islam__${localStorage.getItem("token")}`;
        try {
            await axios.delete(`http://localhost:3002/api/v1/TakafulAccidentReport/delete/${id}`, { // افترضت هذا المسار
                headers: { token }
            });
        ...
        <div className="px-4 py-7  dark:bg-dark2 dark:text-dark3" style={{ minHeight: '100vh' }}>
            ...
            <div className="bg-[rgb(255,255,255)] dark:bg-navbarBack flex p-[22px] rounded-md justify-between items-center mb-3">
                <div className="flex gap-[14px]">
                    ...
                    <a className href="/TakefulRep" data-discover="true"> Takeful Report</a>
                </div>
                <button
                    onClick={() => {
